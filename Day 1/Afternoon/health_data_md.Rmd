---
title: "Andrew's Two-Day R Course - Day One"
...
Welcome to the afternoon session for Day 1. In this lab, you're going to have a go at building your own script using RStudio and doing some data wrangling and visualisation of a dataset.
...
<br>
<br>
Before you start, remember to make a new project file - call it something like "Day_1".  Save the project folder somewhere sensible (maybe create a new folder called something like
...
<br>
<br>
We're going to use a couple of packages in this workshop - the Tidyverse packages, the new gganimate package, and the NHANES package (which contains the NHANES dataset you're going to use this afternoon).  If you haven't used or installed these packages yet, you need to do that first by typing into the console:
...
I then add a geom_boxplot() layer to create a boxplot for each level of our Education factor - and I'm arranging the order in which they're displayed by using the reorder() function to do the re-ordering based on the median BMI for the factor Education. 

The guides(colour = FALSE) function supresses displaying the colour legend - delete it and rerun the code to see what changes.
...
In the above graph, notice that the same y-axis scale is used for each plot - this makes comparisons a little tricky as there are different numbers of cases for each Eduction level. Instead of generating the histograms using a count, we could generate them using a density function. Let's also add a density curve.
...
We can now combine all of the above to examine the effects of Education level and Employment status on BMI as a function of race...
...
Note that the y-axis labels have the same order for each graph - how might you replot these graphs so that the y-axes labels are reordered by descending median BMI for each race?

Have a go at simplifying the code above so we're facet plotting by Race but just considering employment status.
...
Finally, we're going to create some animated graphs using the new gganimate package. This version isn't yet on CRAN so you need to download it from GitHub using the devtools package as follows:
...
These graphs are great as they can be saved as animated gif files and can then used in presentations. To save the last animated plot you built, you can use the anim_save() function such as > anim_save(filename = "my_plot.gif")
